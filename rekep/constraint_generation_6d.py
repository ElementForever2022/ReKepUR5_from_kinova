...
class ConstraintGenerator:
    def __init__(self, config):
        ...
        self.client = OpenAI(base_url='https://api.chatanywhere.tech',
        api_key='sk-01pKrNJFlwstGXg23sXhc2LrDdk8XSRlKM43BnCGIDIkZRDc',)
        self.base_dir = './vlm_query'
        with open(os.path.join(self.base_dir, 'prompt_template_6d.txt'), 'r') as f:
    ...
    def _parse_other_metadata(self, output):
        ...
        grasp_keypoints_template = "grasp_poses = {grasp_keypoints}"
        #这里识别的是grasp_poses而不是grasp_keypoints
        ...
        release_keypoints_template = "release_poses = {release_keypoints}"
        #这里识别的是release_poses而不是release_keypoints
    ...
    def generate(self, img, instruction, metadata):
        ...
        stream = self.client.chat.completions.create(model=self.config['model'],
                                                        ...
                                                        stream=True)
        print(f"Querying OpenAI API for constraints...")
        print(stream)
        output = ""
        start = time.time()
        for chunk in stream:
