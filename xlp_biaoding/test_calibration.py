...
def get_matrix(color_image, aligned_depth_frame):
    ...
    directory = "/home/ur5/rekep/ReKepUR5_from_kinova/xlp_biaoding/img"
    image_names = os.listdir(directory)
    image_paths = ["/home/ur5/rekep/ReKepUR5_from_kinova/xlp_biaoding/img/0.png"]+[os.path.join(directory, name) for name in image_names]  # 标定图片路径
    # image_paths =[os.path.join(directory, name) for name in image_names]  # 标定图片路径

    # 配置参数
    pattern_size = (7, 7)  # 棋盘格内部角点数量，行数为9，列数为12
    square_size = 0.016667  # 棋盘格每格的边长，单位为米（10mm -> 0.01m）
    ...
    #     [0, 0, 0, 1]
    # ])
    r2b_rvecs = np.array([-50, 0, 0])
    r2b_tvecs = np.array([ 0, 0, -90])
